...
ITEM = "pit_wall_unit"   # derived from pit_wall_unit.py through itemkit
...
# ===========================================================================
# ITEMKIT SHIM -- everything below this line used to be re-typed in this file.
# Deleted here and imported instead: hash01, Rng, _h2, _sstep, vnoise1/2,
# fbm1/2, clamp01, smoothstep, _new_mesh, _shade_by_angle, NT, _coll, purge,
# contract_light, add_camera.  See world/itemkit.py for why each one moved.
# ===========================================================================
...
def purge():
    """Prefix-scoped, so this cannot delete another item's sun (see itemkit)."""
    return K.purge(PFX, COLL)


def _new_mesh(name, verts, quads=None, tris=None, smooth_deg=33.0):
    # recentre=False: `unit_mesh_arrays` already returns vertices local to the
    # unit's own centroid and `build_unit` carries the centroid in the object
    # matrix.  This module obeyed Law 6 by hand; itemkit's default makes it
    # automatic for items that have not thought about it yet.
    me, _off = K.new_mesh(name, verts, quads, tris, smooth_deg, recentre=False)
    return me


_shade_by_angle = K.shade_by_angle


def contract_light(scene=None, coll=None):
    """The film's sun and sky. Named with THIS module's prefix, so another
    item's purge() cannot delete it and render this scene black."""
    return K.contract_sun(PFX, scene=scene, coll_=coll)


def add_camera(name, loc, look, lens, coll, fstop=None):
    ob, _dist = K.add_camera(name, loc, look, lens, coll, fstop=fstop)
    return ob
...
def mat_wall():
    ...
    bugbump = t.math("SUBTRACT", 1.0,
                     t.math("MULTIPLY", t.maprange(v_bug, 0.0, 0.10, 1.0, 0.0),
                            t.maprange(t.noise(t.vmath("SCALE", P, scale=1.0),
                                               26.0, 5.0, 0.6),
                                       0.35, 0.75, 0.15, 1.0)))
    # STATED AS RADIANCE MODULATION, NOT AS MILLIMETRES. See itemkit section 5b.
    ...
    # figures and every one had been chosen in millimetres of cloth.
    #
    # These six numbers are NOT a re-tune. Each `modulation_pp` is the value
    # that reproduces the depth this module already shipped and was pixel-peeped
    # at, to the sixth decimal -- `pit_wall_unit --relief` prints the round trip.
    # What has changed is that the module now SAYS WHAT IT IS AIMING AT, so the
    # next agent can argue with 2.63 instead of guessing at 0.0020, and so the
    # depths move if the sun does.
    #
    # The steep ones are deliberate and sit in `RELIEF_BANDS["hard_feature"]`
    # (1.5-6.0): proud aggregate and a bug hole are real voids with real walls,
    # not a crumple, and they are gated by masks (`skin_gone`, `pit`, `splash`)
    # so they act on a fraction of the surface. NOTHING HERE IS ISOTROPIC AT
    # m > 1 -- an ungated field at that depth is the felt this law exists to
    # prevent.
    #
    # THE WAVELENGTHS COME FROM THE SAME CONSTANTS THAT PICKED THE SCALES.
    # Writing 0.0207 here instead of `K.VORONOI_WAVELENGTH_FACTOR / 105.0` would
    # be a second copy of a measured number. Those two have since been
    # independently confirmed twice (itemkit's provenance note is updated).
    #
    # R2-058. `LAM_PLY` USED TO READ `1.0 / 230.0` AND THAT WAS 3.183x TOO LONG.
    # Blender's Wave multiplies the coordinate by 20 before the sine, so a band
    # is 2*pi/20 = 0.31416 of 1/Scale, not 1/Scale — measured flat to six digits
    # over Scale 5..230 (work/wavefix/emitted_wavelength.json). itemkit's
    # `_tex_wavelength_m` had the same error, which is why this line agreed with
    # the audit and both were wrong together.
    #
    # THE DISTANCE ON THE SOCKET HAS NOT MOVED: 0.000240 m, the depth this
    # module shipped and was pixel-peeped at. What changed is that the module
    # now DECLARES what that depth really does to the light. At the true 1.37 mm
    # pitch the same 0.24 mm is a 28.9 deg slope, not a 9.8 deg one, so this
    # stage's real modulation is m 4.370 and was being reported as 1.545.
    # It is still inside RELIEF_BANDS["hard_feature"] (1.5-6.0), which is the
    # right band for a form-liner arris, and it is gated by the `face` mask.
    # Do NOT "fix" this by keeping 1.5452 against the corrected wavelength:
    # that would derive Distance = 0.0000754 m and make the veneer 3.183x
    # SHALLOWER than the surface that was judged. Measured, that change is
    # 21x the render's own noise floor in high-pass energy — see
    # work/wavefix/pixel_table.json.
    ...
    b = t.bump(t.math("ADD", t.math("MULTIPLY", agg_mask, 0.85),
               ...
               modulation_pp=2.6305, wavelength_m=LAM_AGG)    # proud aggregate
    b = t.bump(bugbump, 0.85, normal=b,
               modulation_pp=3.1686, wavelength_m=LAM_BUG)    # small bug holes
    b = t.bump(n_fine, 0.45, normal=b,
               modulation_pp=0.5265, wavelength_m=LAM_PASTE)  # cement paste
    b = t.bump(t.wave(t.vmath("SCALE", Pl, scale=1.0), 230.0, 0.8, 2.0, "Y"),
               t.math("MULTIPLY", face, 0.20), normal=b,
               modulation_pp=4.370320, wavelength_m=LAM_PLY)  # ply veneer
    b = t.bump(t.noise(t.vmath("SCALE", P, scale=1.0), 175.0, 5.0, 0.60),
               t.math("MULTIPLY", pit, 0.60), normal=b,
               modulation_pp=1.8260, wavelength_m=LAM_SAND)   # float sand
    b = t.bump(t.maprange(v_splash, 0.0, 0.3, 1.0, 0.0),
               t.math("MULTIPLY", splash, 0.55), normal=b,
               modulation_pp=1.5479, wavelength_m=LAM_SPLASH)  # splash pitting
...
def test_scene(samples=256, limit=None, quick=False):
    ...
    # THE ONE BEHAVIOURAL CHANGE IN THIS DERIVATION, AND IT IS R2-020.
    # The original set `resolution_x = 1920` here -- as 17 of the 28 wave-1
    # modules do -- while tools/item_gate.py computes every pixel figure from
    # RES_X_4K = 3840. Handing the render settings to itemkit.macro_rig makes
    # 3840x2160 the only thing that comes out unless something else is asked
    # for by name, and it asserts the camera really stands at the manifest's
    # 6.200 m instead of trusting the arithmetic that placed it.
    K.macro_rig(PFX + "CAM_MACRO_4K",
                tuple(float(v) for v in loc), tuple(float(v) for v in aim),
                LENS_MM, cams, scene=scene, samples=samples,
                want_distance_m=FILMED_AT_M, tolerance_m=0.02)
...
def selftest(verbose=True):
    ...
    print("\n[8b] the relief stack is stated in RADIANCE, and it is the same "
          "surface that was pixel-peeped")
    # THE MIGRATION IS A NO-OP ON THE PIXELS, AND THAT IS THE CLAIM UNDER TEST.
    # Section 16 now states each bump stage as a radiance modulation instead of
    # a depth in metres (itemkit 5b). If that changed the depths, it changed the
    # render, and the module would need re-gating. It does not: these are the
    # six Distance values the shipped, peeped build used, reproduced through the
    # law from the modulation each one implies.
    #
    # The negative control is the third row: a modulation 10 % different must
    # NOT reproduce the shipped depth, or the check is measuring nothing.
    #
    # READ THIS BEFORE COPYING THIS CHECK. IT IS HOW R2-058 SURVIVED FOR THREE
    # WEEKS. Every row below states a wavelength and then asks whether
    # `relief_amplitude_for(m, lam)` reproduces a Distance that was itself
    # derived from the SAME `lam`. That is an algebraic identity: it passes for
    # any value of the wavelength constant, including a wrong one, and the ply
    # row used to read `1.0 / 230.0` — 3.183x too long — and passed every time.
    # THE ROUND TRIP IS KEPT because it does catch a mistyped modulation, but it
    # is no longer the only thing here: `[8c]` below RENDERS the wave and counts
    # the bands, which is a measurement the arithmetic cannot fake.
    _V, _N = K.VORONOI_WAVELENGTH_FACTOR, K.NOISE_WAVELENGTH_FACTOR
    _stack = [                       # (name, lam_m, strength, shipped distance)
        ("proud aggregate", _V / 105.0, 1.00, 0.00200, 2.6305),
        ("bug holes",       _V / 190.0, 0.85, 0.00160, 3.1686),
        ("cement paste",    _N / 165.0, 0.45, 0.00040, 0.5265),
        ("ply veneer",      K.WAVE_WAVELENGTH_FACTOR / 230.0,
                                         1.00, 0.00024, 4.370320),
        ("float sand",      _N / 175.0, 1.00, 0.00060, 1.8260),
        ("splash pitting",  _V / 300.0, 1.00, 0.00040, 1.5479),
    ]
    _worst, _ctl = 0.0, 0.0
    for _nm, _lam, _s, _dist, _m in _stack:
        _got = K.relief_amplitude_for(_m, _lam) * 1e-3 / _s
        _bad = K.relief_amplitude_for(_m * 1.10, _lam) * 1e-3 / _s
        _worst = max(_worst, abs(_got - _dist) / _dist)
        _ctl = max(_ctl, abs(_bad - _dist) / _dist)
    chk("stating m reproduces every shipped Distance", _worst < 1e-4 and _ctl > 0.05,
        "worst error over 6 stages %.2e; a 10 %% wrong modulation would miss by "
        "%.1f %%, so the check discriminates (and see [8c]: this row ALONE "
        "cannot fail, because it uses the wavelength constant on both sides)"
        % (_worst, 100 * _ctl))

    print("\n[8c] the ply veneer's wavelength, MEASURED OFF A RENDER")
    # R2-058. The declared 1.37 mm is not an assertion, it is a count of bands
    # in an orthographic emission render of THIS module's own wave node, built
    # by the same call with the same Scale. If `K.WAVE_WAVELENGTH_FACTOR` is
    # ever put back to 1.0, the declared LAM_PLY becomes 4.35 mm, the render
    # still emits 1.37 mm, and this row fails — which is the whole point.
    _lam_ply = K.WAVE_WAVELENGTH_FACTOR / 230.0
    _emit = K.emitted_wavelength_m(
        lambda _t: _t.wave(_t.object_coords(), 230.0, 0.8, 2.0, "X"))
    _cal = K.emitted_wavelength_m(
        lambda _t: _t.wave(_t.object_coords(), wavelength_m=0.0100))
    _err = abs(_emit - _lam_ply) / _lam_ply
    chk("the ply wave EMITS the wavelength this module declares",
        _err < 0.03 and abs(_cal - 0.0100) / 0.0100 < 0.03,
        "declared %.4f mm, rendered and counted %.4f mm, %.2f %% apart "
        "(the 1/Scale reading would be %.4f mm, %.2fx out); the probe's own "
        "control, a 10.000 mm ask, comes back %.4f mm"
        % (1000 * _lam_ply, 1000 * _emit, 100 * _err, 1000.0 / 230.0,
           (1.0 / 230.0) / _emit, 1000 * _cal))
    _rows = K.relief_budget([(n_, l_, d_ * s_ * 1000.0)
                             for n_, l_, s_, d_, _ in _stack], verbose=False)
    chk("no stage is an ungated isotropic field above m = 1",
        all(r["m"] < 1.0 for r in _rows[2:3]),
        "m by stage: %s (the three above 2.6 are proud aggregate, bug holes and "
        "-- since R2-058 corrected its wavelength, not its depth -- the ply "
        "veneer at 4.370; all three are gated by the skin_gone/pit/face masks "
        "and all three sit in RELIEF_BANDS['hard_feature'] 1.5-6.0); "
        "sun %.2f deg, amplifier %.2fx"
        % ([round(r["m"], 2) for r in _rows], K.sun_elev_deg(), K.sun_amplifier()))
