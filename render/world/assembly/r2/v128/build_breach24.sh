#!/bin/bash
# film24_breach = the breach applied to film24, WITH THE FINES.  R2-3361.
#
# v127/build_breach23.sh with W -> work/r23361 and the film23 literals renamed.
# THE 10.9 GB BAKE IS NOT REDONE AND MUST NOT BE: `sim/breachlib.py` reads the
# car only for beat 3 (f865-1056), and the driver-car rebuild is measured at
# EXACTLY 0.0 on every channel across that span (R2-3361 confinement table,
# row `3_breach`).  `sim/out/breach_film.npz` is re-used unchanged.
#
# The original header follows, unedited, because its reasoning is unchanged:
#
...
OUT=${2:-render/film24_breach.blend}
NPZ=sim/out/breach_film.npz
mkdir -p $W

[ -s "$IN" ]  || { echo ">> STAGE RESULT: BREACH24_FAIL (no $IN)"; exit 2; }
[ -s "$NPZ" ] || { echo ">> STAGE RESULT: BREACH24_FAIL (no $NPZ)"; exit 2; }
[ -s world/breach_fines.blend ] || {
  echo ">> STAGE RESULT: BREACH24_FAIL (no fines lib)"; exit 2; }

{
  echo "=== INPUTS, hashed at $(date -Is) ==="
  sha256sum "$NPZ" world/breach_fines.blend sim/apply_breach.py sim/fracture.py
} > $W/inputs_breach24.txt 2>&1
cat $W/inputs_breach24.txt
...
grep -aE "east frame|fines|built [0-9]+ objects|census|Saved as|wrote render|preflight|Traceback|Error" \
    $W/breach24.log | tail -30
ls -la "$OUT" 2>&1

python3 - "$W/apply_film24.json" <<'PY'
import json, sys
try:
    r = json.load(open(sys.argv[1]))
except Exception as e:
    print(">> STAGE RESULT: BREACH24_FAIL (no report: %s)" % e); raise SystemExit(1)
ok = True
st = r["stats"]
...
print(">> STAGE RESULT: %s" % ("BREACH24_BUILT" if ok else "BREACH24_FAIL"))
