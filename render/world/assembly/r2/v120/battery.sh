#!/usr/bin/env bash
# THE #53 GATE BATTERY, run on the world rebuilt against world_contract 1.2.0.
# Every gate here is accompanied by a control run that is KNOWN to fail and one
# that is KNOWN to pass. Serial on purpose: one 4 GB scene at a time on 11 GB.
#
...
while ! grep -q '"blend_mb"' $D/assemble5.log 2>/dev/null; do sleep 15; done
sleep 5
echo "########## BATTERY START $(date +%T)  scene=$S"
ls -l $S

source "$D/lib_battery.sh"

regenerate_controls "$B" "$V/make_controls.py" "$V"
...
expect fail    "placement_gate POSITIVE control (obstacle ON the racing line)" \
  $B -b $V/ctl_place_pos.blend --factory-startup -P $R2/tools/placement_gate.py -- \
     --out $V/ctl_place_pos.json
expect pass    "placement_gate FAR negative control (obstacle 3 km off)" \
  $B -b $V/ctl_place_neg.blend --factory-startup -P $R2/tools/placement_gate.py -- \
...
expect pass    "placement_gate NEAR-MISS negative control (just outside)" \
  $B -b $V/ctl_place_nearmiss_neg.blend --factory-startup -P $R2/tools/placement_gate.py -- \
...
run "placement_gate (shipped default allow)" \
  $B -b $S --factory-startup -P $R2/tools/placement_gate.py -- \
     --out $V/placement_v120.json
run "placement_gate (+ barrier ground objects)" \
  $B -b $S --factory-startup -P $R2/tools/placement_gate.py -- \
     --out $V/placement_v120_ground.json \
...
expect vacuous "collision_gate on the world (no CAR: must REFUSE)" \
  $B -b $S --factory-startup -P $R2/tools/collision_gate.py -- --out $V/collision_v120.json
expect fail    "collision_gate POSITIVE control (car through SHOW_Wall)" \
  $B -b $V/ctl_collide_pos.blend --factory-startup -P $R2/tools/collision_gate.py -- --out $V/ctl_collide_pos.json
expect pass    "collision_gate NEGATIVE control (car clear of everything)" \
  $B -b $V/ctl_collide_neg.blend --factory-startup -P $R2/tools/collision_gate.py -- --out $V/ctl_collide_neg.json

expect vacuous "depth_probe on the world (no CAR: must REFUSE)" \
  $B -b $S --factory-startup -P $R2/tools/depth_probe.py -- --frames 1 --out $V/depth_v120.json
expect fail    "depth_probe POSITIVE control (wheel 200 mm INTO the deck)" \
  $B -b $V/ctl_depth_pos.blend --factory-startup -P $R2/tools/depth_probe.py -- --frames 1 --out $V/ctl_depth_pos.json
expect pass    "depth_probe NEGATIVE control (wheel in CONTACT, 0.0 mm)" \
  $B -b $V/ctl_depth_neg.blend --factory-startup -P $R2/tools/depth_probe.py -- --frames 1 --out $V/ctl_depth_neg.json
...
expect fail    "depth_probe FLOAT positive control (wheel 200 mm ABOVE deck)" \
  $B -b $V/ctl_depth_float_pos.blend --factory-startup -P $R2/tools/depth_probe.py -- --frames 1 --out $V/ctl_depth_float_pos.json

# ------------------------------------------- module-boundary triangle sweep -
run "probeD  module-boundary BVH" $B -b -noaudio $S --factory-startup -P $D/probeD.py -- --out $V/probeD_v120.json
run "probeG  the pairs D found, measured" $B -b -noaudio $S --factory-startup -P $D/probeG.py -- --out $V/probeG_v120.json

# ------------------------------------------------- ground-referenced road ---
run "probe_roadclear (ground-referenced corridor)" \
  $B -b -noaudio $S --factory-startup -P $V/probe_roadclear.py -- $V/roadclear_v120.json

# --------------------------------------------------- the 9 original probes --
run "probeC  P1 P2 P3 P9" $B -b -noaudio $S --factory-startup -P $D/probeC.py -- --out $V/probeC_v120.json
run "probeB  P5 P6 P7 P8 + D2 D3" $B -b -noaudio $S --factory-startup -P $D/probeB.py -- --out $V/probeB_v120.json
run "probeE  P4 barrier feet" $B -b -noaudio $S --factory-startup -P $D/probeE.py -- --out $V/probeE_v120.json
run "probeA  D1 D4 D5 P4" $B -b -noaudio $S --factory-startup -P $D/probeA.py -- --out $V/probeA_v120.json
...
PYCAM
  $B -b $S --factory-startup -P $R2/tools/instance_variety.py -- --out $V/instance_variety_v120.json \
     --path $R2/render/film24_path.json
expect fail    "instance_variety POSITIVE control (1 mesh, 500 instances)" \
  $B -b $V/ctl_variety_pos.blend --factory-startup -P $R2/tools/instance_variety.py -- --out $V/ctl_variety_pos.json \
     --path $V/ctl_variety_path.json
expect pass    "instance_variety NEGATIVE control (500 meshes, 500 inst)" \
  $B -b $V/ctl_variety_neg.blend --factory-startup -P $R2/tools/instance_variety.py -- --out $V/ctl_variety_neg.json \
     --path $V/ctl_variety_path.json
run "variety_distribution (per-family sources)" \
  $B -b $S --factory-startup -P $V/variety_distribution.py -- $V/variety_distribution_v120.json
run "mesh_reuse" $B -b $S --factory-startup -P $R2/tools/mesh_reuse.py

# ------------------------------------------------------- #47 / #48 / #50 ---
run "probe_pitexit on the rebuild" \
  $B -b $S --factory-startup -P $D/probe_pitexit.py -- $V/pitexit_v120.json
