...
merged backing-strip object. Every panel is the same 5.8 mm laminate, so a free
edge rolls over on the same 2.9 mm radius wherever it appears. The nose cone /
chassis joint at x = 2.000 is the one CLOSED joint: both sides stop 0.9 mm short
of the station and roll on a 1.3 mm lip that meets its opposite number on the
joint plane, so what shows outside is a 1.8 mm shut line with a bottom 1.2 mm
down, not an open seam (D-MB45).
...
-----------------------------------------------------------------
    ...
    D-MB45  A HOLE IN THE CAR at the nose cone joint, and the arithmetic that
            put it there. Every fore-aft joint set its two panels hx = GAP/2 +
            T_SKIN/2 = 3.8 mm either side of the station, which spaces the
            CROWNS of the two rolled edges the intended 1.8 mm apart - but the
            crown of a semicircular rollover sits half a laminate BELOW the
            skin, so the two skins themselves ended up 7.6 mm apart. Measured
            round the section at x = 2.000: outer skin to 1.9962, a dish
            falling away from there, and from 1.9988 to 2.0004 nothing at all
            in front of MB_backing 9.1 mm down. Ray-swept at 240 stations round
            the full perimeter, EVERY station was open - the suspension pad
            that plugs it at the FL lower-forward pickup was hiding one station
            out of 240. On the crown it read as a black gash with the weave of
            the backing strip lit at the bottom of it, straight across the
            top-down silhouette.
            The nose joint is now CLOSED. Both sides stop SEAM_HX = GAP/2 short
            of the station and their border rolls on prof_seam instead: a
            quarter round of SEAM_R = 1.3 mm over the edge, then a wall that
            tucks back in to the panel's own inner surface. SEAM_R > SEAM_HX,
            so the two lips MEET on the joint plane 1.3 mm under the skin -
            a bonded butt joint, the same engagement the underpan and the
            chassis already have along their longitudinal joint. Measured
            after: worst lip-to-lip gap anywhere round the perimeter 1.898 mm
            -> 0.235 mm, 0 -> 3088 overlapping triangle pairs, unskinned band
            3.80 mm -> 0.00 mm, deepest thing a ray can see 9.11 mm -> 1.22 mm.
            Spec-surface deviation is unchanged at 0.0087 m (devcheck, 21 894
            samples) and no new interpenetration with any neighbouring module.
            Only this joint is closed, because only this joint is a bonded
            structural butt joint. The other three breaks are the edges of
            REMOVABLE bodywork - sidepod, engine cover, tail fairing - where a
            gap with the backing strip behind it is the intended reading, and
            they keep the plain rollover. They do measure the same way (open
            stations of 240 / widest unskinned band / deepest a ray can see:
            x=0.960  206, 4.00 mm, 11.25 mm; x=-0.440  68, 3.00 mm, 9.22 mm;
            x=-1.450  176, 3.80 mm, 9.13 mm) and none of them was raised by the
            connection audit, so they are recorded here rather than changed.
...
def build_panel(name, coll, rows, cols, mats, thick=T_SKIN, feats=(), cross=(),
    ...
    def dir_for(key, force=None):
        ...
        # against 0.24 mm at worst anywhere else - so a seam lip always rolls
        # square across its own joint.
...
def build(coll, ctx=None):
    ...
    nose_rows = make_rows(X_NOSE_J + SEAM_HX, X_TIP - TIPL, extra=[2.60, 2.64],
    ...
    made.append(build_panel(
        ...
        shrink=nose_shrink, seam_rows=(0,),
    ...
    tf_rows = make_rows(X_SP_LE + hx, X_NOSE_J - SEAM_HX, lines=tf_x)
    ...
    made.append(build_panel(
        ...
        cross=tf_x, seam_rows=(len(tf_rows) - 1,)))
    ...
    up_rows = make_rows(X_TAIL, X_NOSE_J - SEAM_HX, step=0.012, lines=up_x)
    ...
    made.append(build_panel(
        ...
        cross=up_x, seam_rows=(len(up_rows) - 1,)))
