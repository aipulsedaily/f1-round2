...
def _centre_lock(zf, coll, prefix):
    ...
    # ---- axle: hub nut -> wheel-nut bore ---------------------------------- #
    # D-R3-03: this used to be a 24 mm slug floating in mid-air at the dead
    # centre of the wheel face - r 0.0142 sitting inside a r 0.0165 nut bore, so
    # 2.291 mm of clear air all round it, ZERO overlapping triangles with any
    # object in the car, and 72.3 mm short of the hub nut behind it.  Inside the
    # nut's 2 mm-deep boss recess that annulus reads as a dark eccentric
    # crescent ringing a pale stub, dead centre of the front wheel in
    # front-quarter and low-hero framing.
    #
    # It is now an actual axle.  Two engagements, both measured:
    #   * inboard  - r 0.0142 shaft down the 32 mm wheel-centre bore, ending
    #     2.0 mm inside brake_assembly_*_Nut's end cap at HUB_NUT_Z;
    #   * outboard - AXLE_SPIGOT_R steps up to a 0.3 mm interference fit in the
    #     0.0165 nut bore, and the face is brought up flush with the bore mouth
    #     behind a 1.2 mm chamfer so there is no annular air gap left to shade.
    AXLE_SHAFT_R = 0.0142               # clears the 0.0320 wheel-centre bore
    AXLE_SPIGOT_R = 0.0168              # 0.0165 nut bore + 0.3 mm interference
    stub = [(0.0000, HUB_NUT_Z - 0.0020),
            (0.0104, HUB_NUT_Z - 0.0020),
            (AXLE_SHAFT_R, HUB_NUT_Z + 0.0014),
            (AXLE_SHAFT_R, seat - 0.0060),
            (AXLE_SPIGOT_R, seat - 0.0034),
            (AXLE_SPIGOT_R, seat + 0.0142),
            (0.0164, seat + 0.0148),
            (0.0156, seat + 0.0151),
            (0.0080, seat + 0.0152),
            (0.0000, seat + 0.0152)]
...
PEG_R = 0.0068
# PCD clamped so the WHOLE footprint stays on the bell's drive-face annulus,
# whatever anyone retunes it to; 0.0550 also straddles the hub flange OD
# (0.0560) at the root and stays 2.4 mm inside the bell's bolt drillings.
PEG_PCD = min(max(0.0550, BELL_FACE_R[0] + PEG_R), BELL_FACE_R[1] - PEG_R)
PEG_TH0 = HUB_BOSS_TH - math.radians(30.0)   # midway between the hub's bosses
PEG_BASE_Z = HUB_FLANGE_Z + 0.0050           # root sunk in the hub flange rim
PEG_BITE = 0.0070                            # depth into the bell drive face


def _drive_pegs(name, coll):
    """Six drive dowels: hub flange -> disc-bell outboard drive face.

    D-R3-12: these used to run inboard from z = -0.0836 to -0.1049, which is
    49.9 mm PAST the outboard end face of the upright (UPRIGHT_FACE_Z).  A part
    that turns with the wheel had 147 overlapping triangles at 0.007 mm inside
    the one part in the corner that must never turn, while missing every
    rotating part it was supposed to drive - 37.646 mm to the hub, 36.071 mm to
    the disc bell, 37.826 mm to the disc, zero overlaps anywhere.

    They now span the two parts the auditor named.  The root is sunk into the
    hub flange (r 0.0300-0.0560, z 0.0050-0.0112), the nose bears PEG_BITE into
    the bell's outboard drive face at BELL_FACE_Z, and the whole dowel stops
    31.4 mm short of the upright.  Note the wheel centre is a solid drum here
    and simply encloses all of this, so an immersed peg reads as FLOATING to a
    nearest-surface query no matter how deep it is - the load path has to be
    made through surfaces that actually cross, which is why the root is in the
    flange and not just parked in the middle of the drum.

    Two clockings had to be respected: PEG_TH0 = 0 puts the dowels midway
    between the hub's own six drive bosses at HUB_BOSS_TH, and a 55 mm PCD
    keeps them 12.7 mm inside the disc bore and 2.4 mm inside the bell's bolt
    drillings, so nothing else in the corner is touched.
    """
    V, F = [], []
    seg = 30
    b = PEG_BASE_Z
    # the max() is a guard, not a fudge: a deeper PEG_BITE must never be able to
    # walk the nose back across UPRIGHT_FACE_Z into the non-rotating upright.
    tip = max(BELL_FACE_Z - PEG_BITE, UPRIGHT_FACE_Z + 0.0100)
    shank = tip + 0.0049                # where the full-radius shank ends
    prof = [(0.0000, b), (0.0056, b - 0.0004), (0.0066, b - 0.0014),
            (PEG_R, b - 0.0030), (PEG_R, shank), (0.0066, shank - 0.0016),
            (0.0060, shank - 0.0029), (0.0050, shank - 0.0039),
            (0.0036, shank - 0.0045), (0.0019, shank - 0.0048),
            (0.0000, tip)]
    for k in range(6):
        th = TAU * k / 6.0 + PEG_TH0
        cx, cy = PEG_PCD * math.cos(th), PEG_PCD * math.sin(th)
