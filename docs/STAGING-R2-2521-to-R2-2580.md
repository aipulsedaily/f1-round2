...
**The headline is that #78 was already done**, in commit `cc993bc` by
...
Contract selftest `[19]` (added by `cc993bc`) asserts exactly the right thing —
...
`cc993bc` corrected the literal and added a six-quantity diff against the
...
(the field `cc993bc` added — absent means the run predates the fix):
...
`cc993bc` fixed the copy it was pointed at. Sweeping for the numbers rather than
for the name:
...
|---|---|---|
| stale pads | yes, latent | **already fixed** in `cc993bc`; verified; no action |
| unreachable fallbacks | **no** | redundant by design, guarded by `[19]` with two firing controls. A defended null |
| the gate's private car box | **yes, and it was live** | fixed in `cc993bc`; guard **observed to fail** here; 46 of 48 reports on disk are suspect; the copy should still be deleted, not merely corrected |
